...
@objc class TempiFFT : NSObject {
    ...
    private var complexBuffer:COMPLEX_SPLIT
    ...
    init(withSize inSize:Int, sampleRate inSampleRate: Float) {
        ...
        assert(tempi_is_power_of_2(n: inSize), "size must be a power of 2")
        ...
        let mallocSize: Int = Int(halfSize) * MemoryLayout<Float>.size
        let realp = UnsafeMutablePointer<Float>.allocate(capacity: (mallocSize))
        let imagp = UnsafeMutablePointer<Float>.allocate(capacity: (mallocSize))
        self.complexBuffer = DSPSplitComplex(realp: realp, imagp: imagp)
...
//        free(complexBuffer.imagp)
    ...
    func calculateLinearBands(minFrequency: Float, maxFrequency: Float, numberOfBands: Int) {
        assert(hasPerformedFFT, "*** Perform the FFT first.") // note that self.magnitudes will be non-nil
        ...
        for i in 0..<numberOfBands {
            ...
            if magsEndIdx == magsStartIdx {
                ...
                magsAvg = fastAverage(array: self.magnitudes!, magsStartIdx, magsEndIdx)
            }
            self.bandMagnitudes[i] = magsAvg
            self.bandFrequencies[i] = self.averageFrequencyInRange(startIndex: magsStartIdx, magsEndIdx)
        }
    ...
    func calculateLogarithmicBands(minFrequency: Float, maxFrequency: Float, bandsPerOctave: Int) {
        ...
        for i in 0..<octaveBoundaryFreqs.count - 1 {
            ...
            let mags = self.magsInFreqRange(lowFreq: lowerFreq, upperFreq)
            ...
            for j in 0..<bandsPerOctave {
                ...
                if stopIdx <= startIdx {
                    ...
                    let avg = fastAverage(array: mags, startIdx, stopIdx + 1)
                ...
                self.bandFrequencies.append(self.averageFrequencyInRange(startIndex: startMagnitudesIdx, endMagnitudesIdx))
            }
        }
...
//        }
    ...
    func averageMagnitude(lowFreq: Float, highFreq: Float) -> Float {
        ...
        while curFreq <= highFreq {
            total += magnitudeAtFrequency(inFrequency: curFreq)
    ...
    func sumMagnitudes(lowFreq: Float, highFreq: Float, useDB: Bool) -> Float {
        ...
        var total: Float = 0
        while curFreq <= highFreq {
            var mag = magnitudeAtFrequency(inFrequency: curFreq)
            if (useDB) {
                mag = max(0, TempiFFT.toDB(inMagnitude: mag));
            }
            total += mag
            curFreq += self.bandwidth
        }
