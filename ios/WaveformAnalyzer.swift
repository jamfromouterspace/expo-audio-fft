...
public struct WaveformAnalyzer: Sendable {
    ...
    public func samples(fromAudioAt audioAssetURL: URL, count: Int, qos: DispatchQoS.QoSClass = .userInitiated) async throws -> WaveformAnalysis {
        try await Task(priority: taskPriority(qos: qos)) {
            ...
            return try await waveformSamples(track: assetTrack, reader: assetReader, count: count, fftBands: 6)
        }.value
    }
    ...
    public func samples(fromAudioAt audioAssetURL: URL, count: Int, qos: DispatchQoS.QoSClass = .userInitiated, completionHandler: @escaping (Result<[Float], Error>) -> ()) {
        Task {
            do {
                ...
                completionHandler(.success(samples.amplitudes))
            } catch {
