...
export default function App() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (url && !url.startsWith("expo")) {
        ...
        const metadata = await ExpoAudioFFT.getMetadata(url);
  ...
  const onPickDocument = async () => {
    ...
    await ExpoAudioFFT.load(result.assets[0].uri);
    const metadata = await ExpoAudioFFT.getMetadata(result.assets[0].uri);
    console.log("metadata", metadata);
    if (!metadata.duration) {
      console.warn("ERROR: Failed to load song", metadata);
      setIsInitialized(false);
  ...
  return (
    <View style={styles.container}>
      ...
      {duration ? <AudioSlider duration={duration} /> : null}
      <View
        style={{
          height: 100,
          width: 400,
